...
void error(int _errsig, int signal) {
	int errtype = -1;
	errnum++;
	printf("error %d: line %d: near \"%s\" : ",
		errnum, cnum == 0 ? lnum - 1 : lnum,  prid.c_str());//cnum == 0 ? prllen : cnum + 1,, column %d
	//错误类型
	switch (_errsig) {
	...
	case  PARANUMZERO:				errtype = DONOTHING;	printf("function paranum is zero \n"); break;
